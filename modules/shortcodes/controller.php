...
function pdi_paywall_login_shortcode($atts, $content = null)
{
    ...
    if (!is_user_logged_in()) {
        $redirect_to =  $_REQUEST['referer_url'];
        return do_shortcode("[lrm_form 
            default_tab=\"login\" 
            logged_in_message=\"Você está logado no momento!\"
            redirect_to=\"$redirect_to\"
        ]");
    }
    //    ob_start();
//
////    require_once(PDI_PAYWALL_MODULES_PATH . 'shortcodes/views/login.php');
...
function pdi_paywall_register_shortcode($atts, $content = null)
{
    ...
    if (!is_user_logged_in()) {
        $redirect_to =  $_REQUEST['referer_url'].'?plan='.$_REQUEST['plan'];
        return do_shortcode("[lrm_form 
...
/* adicionar tokens no header da página*/
add_action('admin_head', 'pdi_paywall_const_keys', PHP_INT_MAX);
