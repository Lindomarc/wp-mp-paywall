...
<div class="container">
    <h1 class="mb-5 mt-5">PDI Paywall - Assinantes</h1>
    <table id="table_reader" class="table table-bordered display data-table responsive nowrap">
        <thead>
        <tr>
            <th>Nome</th>
            <th>Email</th>
...
<script>
    jQuery(document).ready(function () {
        ...
        jQuery('#table_reader').DataTable({
            language: {
                url: 'https://cdn.datatables.net/plug-ins/1.12.1/i18n/pt-BR.json'
            },
            ajax: {
                "url": 'l/wp-json/wp/v2/users?roles=reader',
                "type": 'get',
                "beforeSend": function (xhr) {
                    xhr.setRequestHeader('Authorization', 'Bearer <?php echo get_option('_pdi_paywall_payment_pdi_token'); ?>');
                    xhr.setRequestHeader('x-customer-key', '<?php echo get_option('_pdi_paywall_payment_pdi_key'); ?>');
